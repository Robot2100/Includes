...
struct tPoint
{
	...
	tPoint() noexcept : a{ static_cast<flo>(0.0), static_cast<flo>(0.0), static_cast<flo>(0.0) } {}
	tPoint(_T _x, _T _y, _T _z) noexcept : a{ _x,_y,_z } {};
	...
	static _T Scalar(const tPoint<_T> & left, const tPoint<_T> & right) noexcept
	{
		return (left.a[0] * right.a[0] + left.a[1] * right.a[1] + left.a[2] * right.a[2]);
	}
	static tPoint<_T> Vector(const tPoint<_T> & left, const tPoint<_T> & right) noexcept
	{
	...
	tPoint<_T> operator-() const noexcept
	{
		return tPoint<_T>(-a[0], -a[1], -a[2]);
	}
	tPoint<_T> operator+(const tPoint<_T> & right) const noexcept
	{
		return tPoint<_T>(a[0]+right.a[0], a[1]+right.a[1], a[2]+right.a[2]);
	}
	tPoint<_T> operator+(const _T b) const noexcept
	{
	...
	bool operator==(const tPoint<_T> & right) const noexcept
	{
...
template<class _T = flo>
tPoint<_T> operator+(const _T b, const tPoint<_T> & right) noexcept
{
...
tPoint<_T> operator*(const tMatrix<_T> & left, const tPoint<_T> & right) noexcept {
	...
	//	throw std::invalid_argument("tPoint<_T>::operator*(tMatrix<_T>,tPoint<_T>) : Matrix must have size 3x3.");
	tPoint<_T> res;
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
