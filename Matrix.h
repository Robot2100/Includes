...
private:
	...
	void resize(const unsigned int a, const unsigned int b) {
		if(a <= 0 || b <= 0)
			throw Error("tMatrix<_T>::resize(int,int) : Parameters must be greater than zero.", Error::EXIT | Error::ERROR);
		size_a = a;
		size_b = b;
		A.resize(a);
		for(int i = 0; i < a; i++) {
	...
	_T HorDet(const unsigned int line) {
		int mod;
		_T det = 0;
		if(line%2 == 1) mod = -1;
		else mod = 1;
		for(int i = 0; i < size_b; i++, mod*=(-1)) {
			if(A[line][i] == (_T)0) continue;
			det+=mod*A[line][i]*((Minor(line,i)).Det());
		}
		return det;
	}
	_T VerDet(const unsigned int line) {
		int mod;
		_T det = 0;
		if(line%2 == 1) mod = -1;
		else mod = 1;
		for(int i = 0; i < size_a; i++, mod*=(-1)) {
...
public:
	...
	inline _T & El(const unsigned int a, const unsigned int b) {
		if(a >= size_a || b >= size_b)
			throw Error("tMatrix<_T>::El(int,int) : Parameter is out of the range.", Error::EXIT | Error::ERROR);
		return A[a][b];
	}
	inline const _T & El(const unsigned int a, const unsigned int b) const {
		if(a >= size_a || b >= size_b)
			throw Error("tMatrix<_T>::El(int,int) : Parameter is out of the range.", nsError::EXIT | nsError::ERROR);
		return A[a][b];
	}
	tMatrix<_T> operator*(const tMatrix<_T> & right) {
		if(this->size_b != right.size_a)
			throw Error("tMatrix<_T>::operator*(tMatrix<_T>,tMatrix<_T>) : Wrong sizes of matrixes.",Error::EXIT | Error::ERROR);
	...
	std::vector<_T> operator*(const std::vector<_T> & right) {
		if(this->size_b != right.size())
			throw Error("tMatrix<_T>::operator*(tMatrix<_T>,std::vector<_T>) : Wrong size of matrix or vector.",nsError::EXIT | nsError::ERROR);
	...
	tMatrix<_T> Minor(const unsigned int a, const unsigned int b) const {
		if(a >= size_a || b >= size_b)
			throw Error("tMatrix<_T>::Minor(int,int) : Parameter is out of the range.", Error::EXIT | Error::ERROR);
		if(size_a <= 1 || size_b <= 1)
			throw Error("tMatrix<_T>::Minor(int,int) : Cannot take a minor of size(1) matrix.", Error::EXIT | Error::ERROR);
	...
	_T Det(const unsigned int line = 0, const bool horisontaly = true) const {
		if(size_a != size_b)
			throw Error("tMatrix<_T>::Det(int=,bool=) : Matrix must be square.", Error::EXIT | Error::ERROR);
		switch(size_a) {
		case 1:
			return A[0][0];
		case 2:
